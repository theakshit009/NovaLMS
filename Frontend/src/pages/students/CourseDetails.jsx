...
function CourseDetails() {
  ...
  const { allCourses, calculateRating, CalculateNoOfLectures, calculateCourseDuration, calculateChapterTimings, currency } = useContext(AppContext)
  ...
  return data ?  ( <>
      ...
      <div className='max-w-xl -mt-15 z-10 text-gray-500'>
        ...
        <div className='pt-8 text-gray-800'>
          ...
          <div className='pt-5'>
            {data.courseContent.map((chapter, ind) => (
        ...
        <div className='py-20 text-smmd:text-default'>
          ...
          <p className='pt-3 rich-text ' dangerouslySetInnerHTML={{ __html: data.courseDescription}}></p>
        </div>
      </div>
      <div className='max-w-[27rem] z-10 shadow-xl -mt-10 rounded-t md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px] '> 
        {
          playerData ? <Youtube videoId={playerData.videoId} opts={{playerVars: { autoplay : 1}}} iframeClassName='w-full aspect-video' />  : <img src={data.courseThumbnail} alt="" />
        }
        <div className='p-5'>
          <div className='flex items-center gap-2'>
          ...
          <div className='flex gap-3 items-center pt-2'>
             <p className='text-gray-800 md:text-4xl text-2xl font-semibold'>
              {currency} {(data.coursePrice - (data.coursePrice * data.discount / 100)).toFixed(2)} 
            </p>
            <p className='md:text-lg text-gray-500 line-through'>{currency} {data.coursePrice}</p>
            <p className='md:text-lg text-gray-500'>{ data.discount}% OFF</p>
          </div>
          <div className='flex items-center text-sm md:text-default gap-4 pt-2 md:pt-4 text-gray-500 '>
            ...
            <div className='flex items-center gap-1'>
              ...
              <p>{calculateCourseDuration(data) }</p>
            </div>
            <div className='h-4 w-px bg-gray-500/40'></div>
            <div className='flex items-center gap-1'>
              <img src={assets.lesson_icon} alt="" />
              <p>{CalculateNoOfLectures(data)} lessons </p>
            </div>
          </div>
          <button className='md:mt-6 mt-4 cursor-pointer w-full py-3 rounded bg-blue-600 text-white font-medium'>{isEnrolled ? 'Already Enrolled' : 'Enroll Now'} </button>
          <div className='pt-6'>
